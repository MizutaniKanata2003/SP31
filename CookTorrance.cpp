...
HRESULT CookTorrance::Init( void )
{
	...
	//シェーダー読み込み
	CreateVertexShader( &m_VertexShader, &m_VertexLayout, "CookTorranceVS.cso" );
	CreatePixelShader( &m_PixelShader, "CookTorrancePS.cso" );

	//3Dオブジェクト管理構造体の初期化
	m_Position = XMFLOAT3( 0.0f - 1.0f, 0.2f, 0.0f );
	...
	m_Parameter.x = 0.3f; // ざらつき
	m_Parameter.y = 0.8f; // 金属感
...
void CookTorrance::Update( void )
{
	...
	ImGui::Begin( "CookTorran" );
	{
		ImGui::SliderFloat( "Roughness", &m_Parameter.x, 0.0f, 1.0f, "％.2f" );
		ImGui::SliderFloat( "Metallic", &m_Parameter.y, 0.0f, 1.0f, "％.2f" );
	}
	ImGui::End();
